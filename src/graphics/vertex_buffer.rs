...
impl<T: Renderable> VertexBuffer<T>
{
    ...
    pub fn get_draw_range_for_uid(&self, uid: &u32) -> Option<&Range<i32>>
    {
        ...
        let range = match self.uid_to_range_map.get(uid)
        {
            Some(r) => r,
            None => {return None;}
        };
    ...
    fn is_range_valid(&self, range: &Range<i32>) -> bool
    {
        //Common sense checks
        if range.start < 0
        {
    ...
    {
        log("We would have freed a renderable here, but it's not implemented yet. TODO!");
    }
    ...
    pub fn buffer_data(&mut self, context: &WebGl2RenderingContext, uid: &u32, vertices: &Vec<f32>, indices: &Vec<u32>)
    {
        ...
        self.uid_to_range_map.insert(uid.clone(),Range::<i32>{ start: index_offset as i32, end: index_offset + index_data.len() as i32});
    }
}
